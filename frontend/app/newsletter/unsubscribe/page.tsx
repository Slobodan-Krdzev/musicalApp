...
function UnsubscribeInner() {
  ...
  useEffect(() => {
    ...
    apiRequest<{ message: string }>(`/api/newsletter/unsubscribe?${params.toString()}`)
      .then((data) => {
        if (!active) return;
        setStatus('success');
      ...
      .catch((err: Error) => {
        if (!active) return;
        setStatus('error');
        setMessage(err.message || 'Unsubscribe failed.');
      });
          ...
          {status !== 'loading' && (
            <div className="mt-8 flex flex-col gap-2">
              ...
              <Link
                href="/parties"
                className={cn(
                  'inline-flex w-full items-center justify-center rounded-lg border border-zinc-700 bg-zinc-900/60 px-4 py-2 text-sm font-medium text-zinc-200 transition-colors hover:bg-zinc-800'
                )}
              >
                Browse parties
              </Link>
